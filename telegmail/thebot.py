...
from bot.classes import User, Connection, Letter
...
from bot.funcs import read_photo, get_user_by_chat_id

from typing import Dict

from APIndital.apindital import get, send, create
    ...
    if result.empty:
        ...
        if column_name == 'namt':
            for row in result.itertuples(index=True):
                r = types.InlineQueryResultArticle(row.id, str(row.namt),
                                                    description=row.code,
                                                      input_message_content=types.InputTextMessageContent(row.namt))
                output.append(r)
        else:
            for row in result.itertuples(index=True):
                r = types.InlineQueryResultArticle(row.id, str(row.code),
...
def get_text_message(message, user):
    if message.text == 'Обратная связь':
        ...
        list_letters = [f'Заявка #{val.id}' for index, val in enumerate(user.sent) if val.sort == 'Заявка']
        ...
        if user.sent != []:
            ...
            for letter in [letter for letter in user.sent if letter.sort == 'Заявка']:
                ticket = get(letter.id)
                body = prepare(ticket)
                if letter.attachs != {}:
...
def prepare(ticket: Dict) -> str:
    """
    Обработать сообщение в содержание сообщения для ТГ
    type ticket: dict - данные о заявке
    rtype: str
    """
    body = (f"Заявка под номером {ticket['id']}\n"
            #f"Тема: {ticket['params']['subject']}\n"
            f"Пост отпраления: {ticket['tp_from']}\n"
            f"Пост назначения: {ticket['tp_to']}\n")
    return body
...
def print_memo(chat_id, markup):
    msg = bot.send_message(chat_id, 
                           ('Ознакомьтесь с тем как прикреплять фотографии документов.\n'
                               '1. Нажать на скрепку\n' 
                               '2. Выбрать файл\n' 
...
def process_attach(message, next_key, user, docum, next_step_handler, step_hndler):
    number = user.number
    key = user.key
...
def send_ticket(message):
    ...
    for indx, letter in enumerate([letter for letter in user.letters if letter.sent == False]): # поменить номер заявки в пуле
        ...
        # create a ticket
        tp_from = get_string(body, 'ления:') # отправления
        tp_to = get_string(body, 'чения:') # назначения
        ticket = create(mail_subj, tp_from, tp_to, phone=user_key)
        status = 404
        if len(user.letters) != 0:
            for file_tuple in files:
                if file_tuple[2] == ('doc'):
                    ...
                    status = send(ticket['id'], downloaded_file, file_name=file_tuple[0])
                    assert status == 200, 'failed sending' 
                elif file_tuple[2] == 'photo':
                    status = send(ticket['id'], file_tuple[1], file_name=file_tuple[0])
                    assert status == 200, 'failed sending'
        ...
        if status == 200 or len(user.letters) != 0:
            letter.id = ticket['id']
            if letter.edit:
                index = user.number
            else:
                index = indx + sent_number
            bot.send_message(message.chat.id, f'Заявка №{letter.id} успешно отправлена.')
            letter.sent = True
            if not letter.edit:
            ...
            list_senderrors.append(ticket) if ticket is not None else list_senderrors
            bot.send_message(message.chat.id, 'Не удалось отправить заявку.')
    if len(list_senderrors) != 0:
